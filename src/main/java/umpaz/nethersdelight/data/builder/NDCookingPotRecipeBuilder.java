...
public class NDCookingPotRecipeBuilder
{
	...
	private NDCookingPotRecipeBuilder(ItemLike resultIn, int count, int cookingTime, float experience, @Nullable ItemLike container) {
	...
	public static NDCookingPotRecipeBuilder cookingPotRecipe(ItemLike mainResult, int count, int cookingTime, float experience) {
		return new NDCookingPotRecipeBuilder(mainResult, count, cookingTime, experience, null);
	}

	public static NDCookingPotRecipeBuilder cookingPotRecipe(ItemLike mainResult, int count, int cookingTime, float experience, ItemLike container) {
		return new NDCookingPotRecipeBuilder(mainResult, count, cookingTime, experience, container);
	}

	public NDCookingPotRecipeBuilder addIngredient(TagKey<Item> tagIn) {
		return addIngredient(Ingredient.of(tagIn));
	}

	public NDCookingPotRecipeBuilder addIngredient(ItemLike itemIn) {
		return addIngredient(itemIn, 1);
	}

	public NDCookingPotRecipeBuilder addIngredient(ItemLike itemIn, int quantity) {
	...
	public NDCookingPotRecipeBuilder addIngredient(Ingredient ingredientIn) {
		return addIngredient(ingredientIn, 1);
	}

	public NDCookingPotRecipeBuilder addIngredient(Ingredient ingredientIn, int quantity) {
		for (int i = 0; i < quantity; ++i) {
	...
	public NDCookingPotRecipeBuilder unlockedBy(String criterionName, CriterionTriggerInstance criterionTrigger) {
		advancement.addCriterion(criterionName, criterionTrigger);
		return this;
	}

	public NDCookingPotRecipeBuilder unlockedByItems(String criterionName, ItemLike... items) {
		return unlockedBy(criterionName, InventoryChangeTrigger.TriggerInstance.hasItems(items));
	}

	public NDCookingPotRecipeBuilder unlockedByAnyIngredient(ItemLike... items) {
	...
	public NDCookingPotRecipeBuilder setRecipeBookTab(CookingPotRecipeBookTab tab) {
		this.tab = tab;
		return this;
	}
	...
	public void build(Consumer<FinishedRecipe> consumerIn) {
		...
		build(consumerIn, NethersDelight.MODID + ":cooking/" + location.getPath());
	}
	...
	public void build(Consumer<FinishedRecipe> consumerIn, ResourceLocation id) {
		if (!advancement.getCriteria().isEmpty()) {
			...
			consumerIn.accept(new NDCookingPotRecipeBuilder.Result(id, result, count, ingredients, cookingTime, experience, container, tab, advancement, advancementId));
		} else {
			consumerIn.accept(new NDCookingPotRecipeBuilder.Result(id, result, count, ingredients, cookingTime, experience, container, tab));
		}
	}
